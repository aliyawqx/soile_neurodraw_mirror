...
import { ArrowLeft, Download, Eraser, Undo, Redo, Save, Brain } from "lucide-react"
...
export default function DrawPage() {
  ...
  const handleDownload = () => {
    ...
    link.download = `neurodraw-${new Date().toISOString().slice(0, 10)}.png`
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      <header className="px-4 lg:px-6 h-16 flex items-center border-b bg-white/80 backdrop-blur-sm">
        ...
        <span className="text-xl font-bold text-blue-900">Нейрорисование</span>
      </header>

      <main className="flex-1 flex flex-col md:flex-row p-4 gap-4">
        <div className="flex-1 flex flex-col gap-4">
          <Card className="p-4 flex-1 flex flex-col bg-white/80 backdrop-blur-sm border-blue-200">
            <div className="relative flex-1 border-2 border-blue-200 rounded-md overflow-hidden bg-white">
              <canvas
                ref={canvasRef}
        ...
        <div className="w-full md:w-80 flex flex-col gap-4">
          <Tabs defaultValue="tools">
            <TabsList className="grid grid-cols-2 bg-blue-100">
              ...
              <TabsTrigger value="guidance" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">
                Подсказки ИИ
              </TabsTrigger>
            </TabsList>
            <TabsContent value="tools" className="space-y-4">
              <Card className="p-4 bg-white/80 backdrop-blur-sm border-blue-200">
                <h3 className="mb-3 text-sm font-medium text-blue-900">Размер кисти</h3>
                <div className="flex items-center gap-2">
                  <Slider
                    ...
                    className="flex-1"
                  />
                  <span className="w-8 text-center text-blue-700 font-medium">{brushSize}</span>
                </div>
              </Card>
              <Card className="p-4 bg-white/80 backdrop-blur-sm border-blue-200">
                <h3 className="mb-3 text-sm font-medium text-blue-900">Цвет</h3>
